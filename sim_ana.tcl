...
#    }
    ...
    dl_local xysacs [smoothsacs $g $valid_sacs 0]
    #dl_local xysacs [smoothems $g]
    dl_local goodems [smoothems $g]
    foreach c "valid_sacs xysacs" {
...
proc show_trial { g trial { show_saccades 0 } { show_post 0 } } {
    ...
    dl_local sactos [dl_select $g:sactos:$trial $valid_sacs:$trial]
...
proc overlap_shuffled_monkey { sessions repetitions } {
    for { set s 0 } { $s < [dl_length $sessions] } { incr s } {
	set session [format %02d [dl_get $sessions $s]]
	cd C:/Users/lab/Documents/MRI_monkey/Glenn/sub-glenn0${session}/ses-01/behavior/task_eyes
	set file_list [glob  l_full*]

	for { set i 0 } { $i < [llength $file_list] } { incr i } { 
	    set filename [lindex $file_list $i]
	    set thisgroup [dg_read ${filename}]
	    overlap_shuffled_repeat $thisgroup "sacs" "ballpos" $repetitions 
	    dg_write $thisgroup shuffled_${filename}
	}
    }
}
...
proc overlap_shuffled_repeat { group signal1 signal2 repetitions { within 1 } { control 1 } } {
	...
	#puts "$i done"
	#set subj [dl_get $currentgroup:subj 0]
	#if { $within == 1 } {
	#    dg_write $currentgroup "L:/projects/analysis/aarit/savedg/shuffled_distributions/within/$subj$i.dgz"
	#} else {
    	#    dg_write $currentgroup "/Volumes/Labfiles/projects/analysis/aarit/savedg/shuffled_distributions/overall/$subj$i.dgz"
	#}
    }
...
proc degreeoverlap_shuffled { g signal1 signal2 { within 1 } { control 1 } } {
	...
	#set returngroup [degreeoverlap [dg_copySelected $groupleft [dl_eq $groupleft:nhit 1]] $signal1 $signal2 $control]
	#for { set i 2 } { $i < 6 } { incr i } {
	#    set currgroup [dg_copySelected $groupleft [dl_eq $groupleft:nhit $i]]
	#    set intersection [degreeoverlap $currgroup $signal1 $signal2 $control]
	#    dg_append $returngroup $intersection
	#}
	...
	#    set currgroup [dg_copySelected $groupright [dl_eq $groupright:nhit $i]]
	#    set intersection [degreeoverlap $currgroup $signal1 $signal2 $control]
	#    dg_append $returngroup $intersection
	#}
	#set returngroup [dg_create]
	#dg_copy $intersection_left returngroup
...
proc degreeoverlap { g signal1 signal2 { control 0 } { translate 0 } { w 256 } { h 256 } } {
    ...
    if { $signal1 == "ems" } {
	dl_local sig1 [smoothems $g $control]
    } elseif { $signal1 == "sacs" } {
	dl_local valid_sacs [dl_between $g:sactimes $g:stimon $g:response]

	for { set i 0 } { $i < [dl_length $valid_sacs] } { incr i } {
	    #puts "trial $i"
	    #puts "first saccade is index [dl_find $valid_sacs:$i 1]"
	    set first_valid_sac [dl_find $valid_sacs:$i 1]
	    if { $first_valid_sac > 0 } {
		set replacement_sac [expr $first_valid_sac -1]
	    } else {
		set replacement_sac 0
	    }
	    dl_set $valid_sacs:$i [dl_replaceByIndex $valid_sacs:$i $replacement_sac 1]
	    #puts "valid sacs start at $replacement_sac"
	}

	dl_local sig1 [smoothsacs $g $valid_sacs $control]
	
	#dl_local sig1 [smoothems $g $control]
    } elseif { $signal1 == "ballpos" } {
	dl_local sig1 [dl_transpose $g:trajectory]
    }

    if { $signal2 == "ems" } {
	dl_local sig2 [smoothems $g $control]
    } elseif { $signal2 == "sacs" } {
	dl_local valid_sacs [dl_between $g:sactimes $g:stimon $g:response]
	dl_local sig2 [smoothsacs $g $valid_sacs $control]
	#dl_local sig1 [smoothems $g $control]
    } elseif { $signal2 == "ballpos" } {
	dl_local sig2 [dl_transpose $g:trajectory]
    }


    for { set trial 0 } { $trial < [dl_length $g:rts] } { incr trial } {
	set trialoverlap [overlap_per_trial $trial $img1 $img2 $sig1 $sig2 $translate $translationx $translationy]
...
proc saccade_planks { g } {
    dl_set $g:saccade_plank_assignment [dl_llist]
...
proc smoothems { g { control 0 } } {
    ...
    dl_local start_inds [dl_div $g:response 5]
    dl_local stop_inds [dl_div $g:endobs 5]
